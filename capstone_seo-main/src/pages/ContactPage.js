...
const ContactPage = () => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        ...
        if (docSnap.exists()) {
          ...
          console.log('Is Admin:', docSnap.data()?.isAdmin);
        }
      } else {
        setUser(null);
        setUserData(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log('Attempting to save contact message:', { name, email, message });
      ...
      if (error.code === 'permission-denied') {
        alert('Permission denied. Please check Firebase security rules for the contactMessages collection.');
      } else {
        alert(`There was a problem sending your message: ${error.message}. Please try again.`);
      }
    }
  };
