...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const loadHeroImages = async () => {
      try {
        const pumaRef = ref(storage, 'Puma.png');
        const nikeRef = ref(storage, 'Nike.png');
        const underarmourRef = ref(storage, 'UnderArmour.png');
        
        const [pumaUrl, nikeUrl, underarmourUrl] = await Promise.all([
          getDownloadURL(pumaRef),
          getDownloadURL(nikeRef),
          getDownloadURL(underarmourRef)
        ]);

        setHeroImages({
          puma: pumaUrl,
          nike: nikeUrl,
          underarmour: underarmourUrl
        });
      } catch (error) {
        console.error('Error loading hero images from Firebase Storage:', error);
      }
    };
      ...
      <section className="bg-light py-5 mb-4">
        <div className="container text-center">
          ...
          <div className="d-flex justify-content-center my-4 gap-4">
            {heroImages.puma && (
              <img src={heroImages.puma} alt="Puma" className="img-fluid" style={{ maxHeight: '60px' }} />
            )}
            {heroImages.nike && (
              <img src={heroImages.nike} alt="Nike" className="img-fluid" style={{ maxHeight: '60px' }} />
            )}
            {heroImages.underarmour && (
              <img src={heroImages.underarmour} alt="Under Armour" className="img-fluid" style={{ maxHeight: '60px' }} />
            )}
          </div>
